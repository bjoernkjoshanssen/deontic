...
\begin{lemma}\label{obSelfSingleton_of_bad}
	\lean{obSelfSingleton_of_bad}
	\leanok
...
\begin{lemma}\label{getAlive}
	\lean{getAlive}
	\leanok
	\uses{local_holds,obSelf_of_obSelf}
	If ob ≠ noObligations and ∀ a, ¬ semibad ob a, then
	    ob = alive k.
\end{lemma}
...
\begin{lemma}\label{obSelf_of_bad}
	\lean{obSelf_of_bad}
	\leanok
	\uses{obSelf_of_bad_not_mem,obSelf_of_bad_mem}
...
Note that Lemma \ref{stayAlive_sub_of_bad} does not require the strong form of C5.
\begin{lemma}\label{stayAlive_sub_of_bad}
	\lean{stayAlive_sub_of_bad}
	\leanok
	\uses{ob_bad,obSelf_of_bad,obSelfSdiff_of_bad}
...
\begin{lemma}\label{stayAlive_of_bad}
	...
	If semibad ob a then ob = stayAlive a.
\end{lemma}
...
\begin{lemma}\label{models_ofCJ_1997}
	...
	\leanok
	\uses{stayAlive_of_bad,getAlive,bad_of_semibad}
